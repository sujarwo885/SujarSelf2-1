...
#cl.login(qr=True)
cl.login(token="tokenEoDvyOxFvEwF8SFQ25Ca.bDQP7xblQtYo/hfeBJ8dUG.EHaUXa9v4YdqeIFbIFXe5vLikibtwbPxnzCgGlW0EZI=")
cl.loginResult()

ki = LINETCR.LINE() #Zorro
#ki.login(qr=True)
ki.login(token="EoLIri1fhNDOrRsJEyS4.ALqq0hix18OmztwW8q1aDa.N52OCWOJeNWCevW4LvmhiJW776IVzMeR+Qq0mWaUiNQ=")
ki.loginResult()

kk = LINETCR.LINE() #Sanji
#kk.login(qr=True)
kk.login(token="Eo8ml4kgVBqCN6xiwtAc.HAXaYczGV0xVj7BkGgemFa.s9CHWe09mVvsGe+oYpT++sxKLA+xaeNs3QkR0RI4/Hw=")
kk.loginResult()

kc = LINETCR.LINE() #Ussop
#kc.login(qr=True)
kc.login(token=" Eo25iUUpFtcSM1Nlp5y6.OsmTA4zIXh1LxjgYbUWI9G.98xQ18RrJfNMDEMy6+Ul+0rHwek7PbqQPKh8YXDIC1g=")
kc.loginResult()

ks = LINETCR.LINE() #Chooper
#ks.login(qr=True)
ks.login(token="EoEn5rjls1wIsEFby57a.DHeHozIH2dDZH7qBMF5JwG.NkF00Qa3Wkgfy3gWhpiJgSrW1w66IYqc8hoZ55nQoY4=")
ks.loginResult()

satpam = LINETCR.LINE() # 
satpam.login(token="EoqtuH5IjWZa6rMbSowd.nklCz8zx3nMDjSP4IRl1Fq.0gzS9w9CbKeANGJgiBpIkZizV0MjfhU6AYbnUcdhPes=")
...
helpMessage ="""✰ Rangers Squad Bots Team✰
Owner : ✰5U74R✰
-==================-
◄]·♦·Menu For Public·♦·[►
[•]Help
...
[•]T-id
-==================-
✰  Rangers Squad Bots Team ✰
-==================-
"""
Keyowner ="""✰ Rangers Squad Bots Team✰
Owner : ✰5U74R✰
-==================-
◄]·♦·Menu For Owner·♦·[►
[•]Kick ...
...
induk=[mid]
Creator=["u577d79f51412f92e7a28d360ca351ae4",mid,Amid,Bmid,Cmid,Dmid,Smid]
admin=["u577d79f51412f92e7a28d360ca351ae4"] #Krisna,kris,
owner=["u577d79f51412f92e7a28d360ca351ae4",mid,Amid,Bmid,Cmid,Dmid,Smid]
wait = {
    'contact':False,
    'autoJoin':True,
            ...
            if not op.param2 in Bots and admin:
              if wait["protectionOn"] == True:
                 try:                    
                    ...
                    kicker.updateGroup(G)
                    Ticket = kicker.kickoutFromGroup(op. param1)
                    kl.acceptGroupInvitationByTicket(op.param1,Ticket)
                    kl.kickoutFromGroup(op.param1,[op.param2])
                    kicker.kickoutFromGroup(op.param1,[op.param2])
                    kl.leaveGroup(op.param1)
...
#====================================================
            ...
            elif msg.text in ["Bot3 open qr","Bot3i buka qr"]:
                if msg.toType == 2:
...
#===============================================================
            ...
            elif "Pp group pict" in msg.text:
              if msg.from_ in admin:
					group = cl.getGroup(msg.to)
...
#==========================================================================		
            ...
            elif "Binvite:on" == msg.text:
              if msg.from_ in admin:
				gid = msg.to
				autocancel[gid] = "poni"
				cl.sendText(msg.to,"ƤƦƠƬЄƇƬ ƖƝƔƖƬƛƬƖƠƝ ƠƝ")
            elif "Bnvite:off" == msg.text:
              if msg.from_ in admin:
				try:
					del autocancel[msg.to]
					cl.sendText(msg.to,"ƤƦƠƬЄƇƬ ƖƝƔƖƬƛƬƖƠƝ ƠƑƑ")
...
#---------------------Sc invite owner ke group------
            elif "/Bawa ke: " in msg.text:
              if msg.from_ in owner:
                gid = msg.text.replace("Bawa ke: ","")
                if gid == "":
                ...
                #print wait2
                 ...
                 if msg.from_ in admin:
                    if msg.to in wait2['readPoint']:
                        ...
                        cl.sendText(msg.to, "||Di Read Oleh||%s\n||By : ✰5U74R✰ ✰  Rangers Squad Bots Team ✰||\n\n>Pelaku CCTV<\n%s-=CCTV=-\n•Bintitan\n•Panuan\n•Kurapan\n•Kudisan\n\nAmiin Ya Allah\n[%s]" % (wait2['readMember'][msg.to],chiya,setTime[msg.to]))
                    else:
                    ...
                    if msg.to in wait2['readPoint']:
                        if wait2["ROM"][msg.to].items() == []:
                            ...
                            for rom in wait2["ROM"][msg.to].items():
                                print "[Command] Check"
                                chiya += rom[1] + "\n"
                        cl.sendText(msg.to, "✔ ✰  Rangers Squad Bots Team ✰\nRead : %s\n\n✖ Sider :\n%s\nPoint creation date n time:\n[%s]"  % (wait2['readMember'][msg.to],chiya,setTime[msg.to]))
                        try:
                            del wait2['readPoint'][msg.to]
                            del wait2['readMember'][msg.to]
                        except:
...
#---------------FUNGSI RATAIN GRUP TANPA KICK SESAMA BOT/Admin/Bots----------#
            elif "Tumpas1" in msg.text:
	      if msg.from_ in admin:
                if msg.toType == 2:
                    print "ok cleanse"
                    _name = msg.text.replace("Tumpas1","")
                    gs = ki.getGroup(msg.to)
                    gs = kk.getGroup(msg.to)
                    gs = kc.getGroup(msg.to)
...
#-----------------------------------------------
         #----------------Fungsi Join Group Start-----------------------#
            elif msg.text in ["Join all","Come"]: #Panggil Semua Bot
              if msg.from_ in owner:
...
#----------------------------[Spam To Contact]----------------------------#WORK 
       ...
       #--------------Fungsi Broadcast Finish-----------#                  
            ...
            elif msg.text in ["LG1"]: #Melihat List Group
              if msg.from_ in admin:
      ...
      #--------------List Group------------
       #------------ Keluar Dari Semua Group------
            elif msg.text in ["Minggat"]: # Keluar Dari Semua Group Yang Di dalem nya  ada bot(Kalo Bot Kalian Nyangkut di Group lain :D)
              if msg.from_ in owner:
                gid = cl.getGroupIdsJoined()
    ...
    for zx in range(0,500):
      ...
      if hasil['result']['posts'][zx]['postInfo']['liked'] == False:
        try:
          ...
          cl.comment(hasil['result']['posts'][zx]['userInfo']['mid'],hasil['result']['posts'][zx]['postInfo']['postId'],"👉 Rangers Squad Bots Team")
          ...
          cl.comment(hasil['result']['posts'][zx]['userInfo']['mid'],hasil['result']['posts'][zx]['postInfo']['postId'],"👉ąµţ๏ℓɨЌ€ By✰ t Rangers Squad Bots Team««")
          print "Like"
        except:
          pass
      else:
        ...
        if hasil['result']['posts'][zx]['postInfo']['liked'] == False:
            if hasil['result']['posts'][zx]['userInfo']['mid'] in owner:
                try:
                    ...
                    cl.comment(hasil['result']['posts'][zx]['userInfo']['mid'],hasil['result']['posts'][zx]['postInfo']['postId'],"👉Rangers Squad Bots Team««")
                    ki.comment(hasil['result']['posts'][zx]['userInfo']['mid'],hasil['result']['posts'][zx]['postInfo']['postId'],"👉 Rangers Squad Bots Team««")
                    kk.comment(hasil['result']['posts'][zx]['userInfo']['mid'],hasil['result']['posts'][zx]['postInfo']['postId'],"👉 Rangers Squad Bots Team««")
                    kc.comment(hasil['result']['posts'][zx]['userInfo']['mid'],hasil['result']['posts'][zx]['postInfo']['postId'],"👉 Rangers Squad Bots Team««")
                    ks.comment(hasil['result']['posts'][zx]['userInfo']['mid'],hasil['result']['posts'][zx]['postInfo']['postId'],"👉Rangers Squad Bots Team««")
                    print "Like"
                except:
                    pass
            else:
